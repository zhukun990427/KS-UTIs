...
###Logistic regression
LR = LogisticRegression(penalty="l2", solver="liblinear", max_iter=5000)
LR.fit(X_train_scal,y_train)
LR.score(X_train_scal,y_train)
LR.score(X_test_scal,y_test)

y_train_prob = LR.predict_proba(X_train_scal)[:,1]
for i in range(len(y_train_prob)):
    print(y_train_prob[i], end=',')
y_train_pred = LR.predict(X_train_scal)
y_test_prob= LR.predict_proba(X_test_scal)[:,1]
