...
for i  in range(0,200,10):
    RF = Classifier(n_estimators=i+1, n_jobs=-1,random_state=42)
    cross_score = cross_val_score(RF, X_test_scal,y_test, cv=5).mean()
...
n_estimators = cross.index(max(cross))+1

param_grid = {'max_depth' : np.arange(1,20,1)}
RF = Classifier(n_estimators=11,random_state=42)
